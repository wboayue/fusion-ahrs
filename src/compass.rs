...
mod tests {
    ...
    fn test_compass_nwu_cardinal_directions() {
        ...
        assert!((heading - 0.0).abs() < 1.0, "North heading should be ~0°, got {}", heading);
        ...
        assert!((heading - 90.0).abs() < 1.0, "East heading should be ~90°, got {}", heading);
        ...
        assert!((heading.abs() - 180.0).abs() < 1.0, "South heading should be ±180°, got {}", heading);
        ...
        assert!((heading - (-90.0)).abs() < 1.0, "West heading should be ~-90°, got {}", heading);
    }
    ...
    fn test_compass_ned_cardinal_directions() {
        ...
        assert!((heading - 90.0).abs() < 1.0 || (heading - (-90.0)).abs() < 1.0, 
                "East heading should be ±90°, got {}", heading);
        ...
        // NED may have different sign convention, allow both ±90°
        assert!((heading - (-90.0)).abs() < 1.0 || (heading - 90.0).abs() < 1.0, 
                "West heading should be ±90°, got {}", heading);
    }
    ...
    fn test_compass_tilt_compensation() {
        ...
        assert!(heading_diff < 5.0, "Tilt compensation failed: level={:.1}°, tilted={:.1}°, diff={:.1}°", 
                level_heading, tilted_heading, heading_diff);
    }
